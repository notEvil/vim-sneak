...
func! sneak#label#mylabel(input, n, reverse)
  ...
  if a:n " sneak
    let n = min([a:n, 999])
    let flags = 'W'.(a:reverse ? 'b' : '')

    while n
      let [l, c] = searchpos(pattern, flags)
      let skippedfold = sneak#util#skipfold(l, a:reverse)
    ...
    let w = winsaveview()
    let restrict_on_screen = s.get_onscreen_searchpattern(w)

    call sneak#hl#removehl()
    let w:sneak_hl_id = matchadd('Sneak', pattern.restrict_on_screen)

    return 1
  endif

  " prepare label
  if !hlexists('SneakCursor') " workaround
    hi! link SneakCursor Cursor
  endif
  call s:before()
  ...
  let restrict_on_screen = s.get_onscreen_searchpattern(w)
  let pattern = pattern.restrict_on_screen
  ...
  " highlight matches
  call sneak#hl#removehl()
  let w:sneak_hl_id = matchadd('Sneak', pattern)
